        ...
        <div class="col">
          <div class="form-group">
            ...
            <template v-if="$v.form.text.$error">
              <small class="text-danger" v-if="!$v.form.text.required">Durum Adı girilmelidir</small>
            </template>
          </div>
        </div>
    ...
    <table class="table table-hover">
      ...
      <tr>
        ...
        <th class="fit text-right">
          <PlusCircleIcon v-b-modal.modal-statusInfo-add-edit/>
        </th>
      </tr>
      </thead>
      <tbody>
      <tr class="cursor-pointer" @click="showEditModal(item)"
          v-for="(item,index) in $store.state.supplierManagement.stateInfoParameters" :key="index">
        <td>{{ item.metin }}</td>
...
export default {
  ...
  data() {
    return {
      stateNameMaxLength: 50,
      modalTitle: 'Yeni Durum Bilgisi Ekle',
      isEditing: false,
      overlay: false,
      form: {
        id: null,
  ...
  methods: {
    ...
    ...mapActions('supplierManagement', ['updateStateInfoParameter', 'addStateInfoParameter', 'getStateInfoParameters']),
    clear() {
      this.modalTitle = 'Yeni Durum Bilgisi Ekle';
      this.$v.$reset();
    ...
    save() {
      ...
      if (!this.$v.$invalid) {
        this.overlay = true;
        if (this.isEditing) {
          this.updateStateInfoParameter(this.form).then((res) => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            } else {
              this.getStateInfoParameters(2);
              this.$bvModal.hide('modal-statusInfo-add-edit');
            }
          }).catch(err => {
            this.overlay = false;
          ...
          this.addStateInfoParameter(this.form).then((res) => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            } else {
    ...
    showEditModal(item) {
      ...
      this.isEditing = true;
      this.modalTitle = 'Durum Bilgisi Güncelleme';
      this.$bvModal.show('modal-statusInfo-add-edit');
    },
