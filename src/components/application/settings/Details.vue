        ...
        <div class="col">
          <div class="form-group">
            ...
            <template v-if="$v.form.text.$error">
              <small class="text-danger" v-if="!$v.form.text.required">{{ this.getMessage(211) }}</small>
            </template>
          </div>
        </div>
    ...
    <table class="table table-hover">
      ...
      <tr>
        ...
        <th class="fit text-right">
          <PlusCircleIcon v-b-modal.modal-details-add-edit/>
        </th>
      </tr>
      </thead>
      <tbody>
      <tr class="cursor-pointer" @click="showEditModal(item)"
          v-for="(item,index) in $store.state.applicationSettings.details" :key="index">
        <td>{{ item.metin }}</td>
...
export default {
  ...
  data() {
    return {
      modalTitle: 'Başvuru Detayı Ekle',
      isEditing: false,
      overlay: false,
      form: {
        id: null,
  ...
  methods: {
    ...
    ...mapActions('applicationSettings', ['updateDetails', 'addDetails', 'getDetails']),
    clear() {
      this.modalTitle = 'Başvuru Detayı Ekle';
      this.$v.$reset();
    ...
    save() {
      ...
      if (!this.$v.$invalid) {
        this.overlay = true;
        if (this.isEditing) {
          this.updateDetails(this.form).then((res) => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            }else{
              this.$bvModal.hide('modal-details-add-edit');
          ...
          this.addDetails(this.form).then((res) => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            }else{
    ...
    showEditModal(item) {
      this.isEditing = true;
      this.modalTitle = 'Başvuru Detayı Düzenle';
      this.form.id = item.id;
      this.form.text = item.metin;
      this.form.passive = item.pasif;
      this.$bvModal.show('modal-details-add-edit');
    }
  },
  created() {
    this.getDetails();
