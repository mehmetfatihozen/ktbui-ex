<template>
  <div>
        ...
        <div class="col">
          <div class="form-group">
            ...
            <template v-if="$v.form.text.$error">
              <small class="text-danger" v-if="!$v.form.text.required">{{ this.getMessage(23) }}</small>
            </template>
          </div>
        </div>
    ...
    <table class="table table-hover">
      <thead>
      <tr>
        <th>Başvuru Tipleri</th>
        <th class="fit">Pasif</th>
        <th class="fit text-right">
          <PlusCircleIcon v-b-modal.modal-type-add-edit/>
        </th>
      </tr>
      </thead>
      <tbody>
      <tr class="cursor-pointer" @click="showEditModal(item)"
          v-for="(item,index) in $store.state.applicationSettings.types" :key="index">
        <td>{{ item.metin }}</td>
...
export default {
  ...
  methods: {
    ...
    ...mapActions('applicationSettings', ['updateType', 'addType', 'getTypes']),
    clear() {
      this.modalTitle = 'Başvuru Tipi Ekle';
      this.$v.$reset();
    ...
    save() {
      ...
      if (!this.$v.$invalid) {
        this.overlay = true;
        if (this.isEditing) {
          this.updateType(this.form).then((res) => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            } else {
              this.$bvModal.hide('modal-type-add-edit');
    ...
    showEditModal(item) {
      this.isEditing = true;
      this.modalTitle = 'Başvuru Tipi Düzenle';
      this.form.id = item.id;
      this.form.text = item.metin;
      this.form.passive = item.pasif;
      this.$bvModal.show('modal-type-add-edit');
    }
  },
  created() {
    this.getTypes();
