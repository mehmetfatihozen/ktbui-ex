        ...
        <div class="col-3">
          <div class="form-group">
            ...
            <select class="form-control" v-if="!isEditing" v-model="form.state" @change="getSubStates"
              ...
              <option :value="item.id" v-for="(item,index) in stateDropdown" :key="index">{{ item.metin }}</option>
            </select>
        ...
        <div class="col">
          <div class="form-group">
            ...
            <select class="form-control" v-if="!isEditing" v-model="form.subState"
              ...
              <option :value="null">Seçiniz</option>
              <option :value="item.id" v-for="(item,index) in subStateDropdown" :key="index">{{ item.metin }}</option>
            </select>
...
export default {
  ...
  methods: {
    ...
    ...mapActions('sp', ['getStates', 'getStateDropdown', 'getSubStateDropdown', 'addState', 'updateState']),
    getSubStates(e) {
      this.getSubStateDropdown({planId: this.$route.params.id, stateId: e.target.value});
    },
    showEditModal(item) {
    ...
    clear() {
      this.$store.state.sp.subStateDropdown = Object.assign([]);
  ...
  created() {
    this.getStateDropdown(this.$route.params.id);
    this.getStates(this.$route.params.id);
  },
  computed: {
    ...mapState('sp', ['states', 'stateDropdown', 'subStateDropdown'])
  }
}
</script>