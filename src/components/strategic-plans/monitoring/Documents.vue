<template>
  <div>
        ...
        <div class="col">
          <div class="form-group">
            ...
            <b-form-select
                ...
                :options="$store.state.sp.monitoringDocumentTypes"
                value-field="id"
                text-field="metin"
                plain
    ...
    <table class="table table-hover">
      ...
      <tbody>
      <tr v-for="(item,index) in $store.state.sp.monitoringDocuments" :key="index">
...
export default {
  ...
  methods: {
    ...
    ...mapActions('sp', ['deleteMonitoringDocument', 'addMonitoringDocument', 'getMonitoringDocumentParameters', 'getMonitoringDocuments']),
    searchEbys() {
    ...
    save() {
      ...
      if (!this.$v.$invalid) {
        ...
        this.addMonitoringDocument(formData).then((res) => {
          this.overlay = false;
          if (res.hasError) {
            this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
          } else {
    ...
    remove() {
      this.overlay = true;
      this.deleteMonitoringDocument(this.selectedId).then((res) => {
        this.overlay = false;
        if (res.hasError) {
          this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
        } else {
  ...
  created() {
    this.getMonitoringDocumentParameters();
    this.getMonitoringDocuments(this.$route.params.id);
  },
  watch: {
    'form.ebys'(nw) {
      if (!nw) {
