<template>
  <div class="d-content">
    <b-modal
        id="modal-add-strategy"
        size="lg"
        centered
        title="Hedef Detay Girişi : STRATEJİLER"
        ok-title="Kaydet"
        ok-variant="success"
        cancel-title="İptal"
        :busy="overlay"
    ...
    >
      <div class="form-group">
        ...
        <template v-if="$v.form.text.$error">
          <small class="text-danger" v-if="!$v.form.text.required">Strateji girilmelidir</small>
        </template>
      </div>
    </b-modal>
    <b-modal
        id="modal-delete-strategy"
        centered
        title="Uyarı"
    ...
    <table class="table table-hover">
      ...
      <tr>
        ...
        <th class="fit text-center">
          <PlusCircleIcon v-b-modal.modal-add-strategy/>
        </th>
      </tr>
      </thead>
      <tbody>
    ...
    remove(){
      ...
      this.deleteGoalDetail(this.selectedItem.id).then(res => {
        ...
        if (res.hasError) {
          ...
          this.$bvModal.hide('modal-delete-strategy');
        }
      }).catch((err) => {
      ...
      if (!this.$v.$invalid) {
        ...
        this.addGoalDetail(this.form).then(res => {
          ...
          if (res.hasError) {
            ...
            this.$bvModal.hide('modal-add-strategy');
          }
        }).catch((err) => {
          this.showModal({title: 'Hata!', message: err, type: 'danger'});
          this.overlay = false;
