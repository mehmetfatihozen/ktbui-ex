<template>
  <div class="d-content">
    <b-modal
        id="modal-add-detection"
        size="lg"
        centered
        title="Hedef Detay Girişi : TESPİTLER"
        ok-title="Kaydet"
        ok-variant="success"
        cancel-title="İptal"
        :busy="overlay"
    ...
    >
      <div class="form-group">
        ...
        <template v-if="$v.form.text.$error">
          <small class="text-danger" v-if="!$v.form.text.required">Tespit girilmelidir</small>
        </template>
      </div>
    </b-modal>
    <b-modal
        id="modal-delete-detection"
        centered
        title="Uyarı"
    ...
    <table class="table table-hover">
      ...
      <tr>
        ...
        <th class="fit text-center">
          <PlusCircleIcon v-b-modal.modal-add-detection/>
        </th>
      </tr>
      </thead>
      <tbody>
...
export default {
  ...
  methods: {
    ...
    showDeleteModal(item) {
      ...
      this.$bvModal.show('modal-delete-detection');
    },
    remove() {
      this.overlay = true;
      this.deleteGoalDetail(this.selectedItem.id).then(res => {
        ...
        if (res.hasError) {
          ...
          this.$bvModal.hide('modal-delete-detection');
        }
      }).catch((err) => {
      ...
      if (!this.$v.$invalid) {
        ...
        this.addGoalDetail(this.form).then(res => {
          ...
          if (res.hasError) {
            ...
            this.$bvModal.hide('modal-add-detection');
          }
        }).catch((err) => {
          this.showModal({title: 'Hata!', message: err, type: 'danger'});
          this.overlay = false;
