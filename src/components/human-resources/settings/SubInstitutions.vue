  ...
  <div>
    ...
    >
      <div class="row">
        <div class="col-8">
          <div class="form-group">
            ...
            <input type="text" class="form-control" disabled  :value="$store.state.hrSettings.selectedTopDepartment.birimAd">
          </div>
        </div>
        <div class="col-4">
          <div class="row">
            <div class="col">
              <div class="form-group">
                ...
                <input type="text" class="form-control" disabled :value="$store.state.hrSettings.selectedTopDepartment.detsisNo">
              </div>
            </div>
            <div class="col">
              <div class="form-group">
                <label>Kurum Kodu</label>
                <input type="text" class="form-control" disabled :value="$store.state.hrSettings.selectedTopDepartment.kurumKodu">
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="row">
        <div class="col-8">
          <div class="form-group">
            ...
            <input type="text" class="form-control" disabled  :value="$store.state.hrSettings.selectedLinkedDepartment.birimAd">
          </div>
        </div>
        <div class="col-4">
          <div class="row">
            <div class="col">
              <div class="form-group">
                <label>DETSİS No</label>
                <input type="text" class="form-control" disabled :value="$store.state.hrSettings.selectedLinkedDepartment.detsisNo">
              </div>
            </div>
            <div class="col">
              <div class="form-group">
                <label>Kurum Kodu</label>
                <input type="text" class="form-control" disabled :value="$store.state.hrSettings.selectedLinkedDepartment.kurumKodu">
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="row">
        <div class="col-8">
          <div class="form-group">
            <label>Alt Birim Adı</label>
            <input type="text" class="form-control"
                   v-model="form.departmentName"
                   :class="{ 'is-invalid': $v.form.departmentName.$error }">
            <template v-if="$v.form.departmentName.$error">
              <small class="text-danger" v-if="!$v.form.departmentName.required">Alt Birim Adı girilmelidir</small>
            </template>
          </div>
        </div>
        <div class="col-4">
    ...
    <div class="row mb-3">
      <div class="col"><h3 class="mb-0">{{ selectedLinkedDepartment.birimAd }}</h3></div>
      <div class="col text-right">
        <button class="btn btn-outline-secondary btn-sm" @click="changeInstitutionComponent({cmp:'LinkedInstitutions',data:{id:selectedLinkedDepartment.id,birimAd:selectedLinkedDepartment.birimAd}})">
    ...
    <table class="table table-hover">
      ...
      <tr>
        ...
        <th class="fit text-right">
          <img src="@/assets/img/icons/plus-circle.png" alt="" v-b-modal.modal-sub-institution>
        </th>
      </tr>
      </thead>
      <tbody>
      <tr class="cursor-pointer" @click="showEditModal(item)"
          v-for="(item,index) in $store.state.hrSettings.subDepartments" :key="index">
        <td>{{ item.birimAd }}</td>
        ...
        <td class="fit text-right">
          <button class="btn btn-outline-secondary btn-parameter mr-2 btn-sm"
                  @click="departments(item)">
            Birimler
          </button>
          <button class="btn btn-outline-secondary btn-parameter btn-sm"
                  @click="changeInstitutionComponent({cmp:'SubInstitutionsHistory',data:item})">
            Tarihçe
          </button>
        </td>
      </tr>
...
export default {
  data(){
    return {
      overlay:false,
      modalTitle: 'Yeni Alt Birim Girişi',
      form: {
        id: null,
        parentId: this.$store.state.hrSettings.selectedLinkedDepartment.id,
        departmentName: '',
        detsis: '',
        departmentCode: '',
        categoryId: null,
  ...
  methods: {
    ...
    ...mapActions('hrSettings', ['getSubDepartments','addSubDepartment','updateSubDepartment']),
    departments(item){
      this.changeInstitutionComponent({cmp:'Institutions',data:{...item}});
      this.$store.state.hrSettings.selectedSubDepartment=Object.assign({},item);
    },
    showEditModal(item) {
      this.modalTitle = 'Alt Birim Düzenle';
      this.isEditing = true;
      ...
      this.$bvModal.show('modal-sub-institution');
    },
    clear() {
      this.modalTitle = 'Yeni Alt Birim Girişi';
    ...
    save() {
      ...
      if (!this.$v.$invalid) {
        ...
        if (this.isEditing) {
          ...
          this.addSubDepartment(this.form).then(res => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            } else {
  ...
  created() {
    this.getSubDepartments(this.$store.state.hrSettings.selectedLinkedDepartment.id);
  },
  computed:{
    ...mapState('hrSettings',['selectedLinkedDepartment'])
  }
}
</script>