<template>
  <div>
      ...
      <div class="row">
        <div class="col-8">
          <div class="form-group">
            ...
            <input type="text" class="form-control" disabled  :value="$store.state.hrSettings.selectedSubDepartment.birimAd">
          </div>
        </div>
        <div class="col-4">
          <div class="row">
            <div class="col">
              <div class="form-group">
                <label>DETSİS No</label>
                <input type="text" class="form-control" disabled :value="$store.state.hrSettings.selectedSubDepartment.detsisNo">
              </div>
            </div>
            <div class="col">
              <div class="form-group">
                <label>Kurum Kodu</label>
                <input type="text" class="form-control" disabled :value="$store.state.hrSettings.selectedSubDepartment.kurumKodu">
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="col-8">
          <div class="form-group">
            ...
            <template v-if="$v.form.departmentName.$error">
              <small class="text-danger" v-if="!$v.form.departmentName.required">Birim Adı girilmelidir</small>
            </template>
          </div>
        </div>
        <div class="col-4">
    ...
    <div class="row mb-3">
      <div class="col-10"><h4 class="mb-0">{{ $store.state.hrSettings.selectedSubDepartment.birimAd }}</h4></div>
      <div class="col text-right">
        <button class="btn btn-outline-secondary btn-sm"
                @click="changeInstitutionComponent({cmp:'SubInstitutions',data:null})">
    ...
    <table class="table table-hover">
      ...
      <tr class="cursor-pointer" @click="showEditModal(item)"
          v-for="(item,index) in $store.state.hrSettings.departments" :key="index">
        <td>{{ item.birimAd }}</td>
        ...
        <td class="fit text-right">
          <button class="btn btn-outline-secondary btn-parameter btn-sm"
                  @click="changeInstitutionComponent({cmp:'InstitutionsHistory',data:item})">
            Tarihçe
          </button>
        </td>
      </tr>
...
export default {
  data() {
    return {
      ...
      form: {
        id: null,
        parentId: this.$store.state.hrSettings.selectedSubDepartment.id,
        departmentName: '',
        detsis: '',
        departmentCode: '',
        categoryId: null,
  ...
  methods: {
    ...
    ...mapActions('hrSettings', ['getDepartments', 'addDepartment', 'updateDepartment']),
    showEditModal(item) {
    ...
    clear() {
      this.modalTitle = 'Alt Birim Girişi';
    ...
    save() {
      ...
      if (!this.$v.$invalid) {
        this.overlay = true;
        if (this.isEditing) {
          this.updateDepartment(this.form).then(res => {
            this.overlay = false;
            if (res.hasError) {
          ...
          this.addDepartment(this.form).then(res => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            } else {
  ...
  created() {
    this.getDepartments(this.$store.state.hrSettings.selectedSubDepartment.id);
  }
}
</script>