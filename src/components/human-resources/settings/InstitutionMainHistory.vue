  ...
  <div>
    <div class="row mb-3">
      ...
      <div class="col text-right">
        <button class="btn btn-outline-secondary btn-sm" @click="changeInstitutionComponent({cmp:'InstitutionMain',data:null})">
    ...
    <table class="table">
      ...
      <tbody>
      <tr v-for="(item,index) in $store.state.hrSettings.topDepartmentHistories" :key="index">
        <td>{{ item.adi }}</td>
...
export default {
  ...
  methods:{
    ...
    ...mapActions('hrSettings',['getTopDepartmentHistory']),
  },
  created() {
    this.getTopDepartmentHistory(this.id);
  }
}
</script>