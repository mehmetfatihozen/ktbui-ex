<template>
  <div>
    <div class="row mb-3">
      <div class="col"><h3 class="mb-0">Tarihçe : {{ birimAd }}</h3></div>
      <div class="col text-right">
        <button class="btn btn-outline-secondary btn-sm" @click="changeInstitutionComponent({cmp:'LinkedInstitutions',data:null})">
    ...
    <table class="table">
      ...
      <tbody>
      <tr v-for="(item,index) in $store.state.hrSettings.linkedDepartmentHistories" :key="index">
        <td>{{ item.adi }}</td>
...
export default {
  ...
  methods:{
    ...
    ...mapActions('hrSettings',['getLinkedDepartmentHistory']),
  },
  created() {
    this.getLinkedDepartmentHistory(this.id);
  }
}
</script>