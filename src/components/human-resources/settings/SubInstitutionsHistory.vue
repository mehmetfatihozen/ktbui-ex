<template>
  <div>
    ...
    <table class="table">
      ...
      <tbody>
      <tr v-for="(item,index) in $store.state.hrSettings.subDepartmentHistories" :key="index">
        <td>{{ item.adi }}</td>
...
export default {
  ...
  methods: {
    ...
    ...mapActions('hrSettings',['getSubDepartmentHistory']),
  },
  created() {
    this.getSubDepartmentHistory(this.id);
  }
}
</script>