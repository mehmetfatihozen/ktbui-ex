  ...
  <div>
    ...
    <table class="table table-hover">
      ...
      <tr class="cursor-pointer" @click="showEditModal(item)"
          v-for="(item,index) in $store.state.hrSettings.employeeDocuments" :key="index">
        <td>{{ item.metin }}</td>
...
export default {
  ...
  methods: {
    ...
    ...mapActions('hrSettings', ['updateEmployeeDocument', 'addEmployeeDocument', 'getEmployeeDocuments']),
    clear() {
    ...
    save() {
      ...
      if (!this.$v.$invalid) {
        this.overlay = true;
        if (this.isEditing) {
          this.updateEmployeeDocument(this.form).then((res) => {
            this.overlay = false;
            if (res.hasError) {
          ...
          this.addEmployeeDocument(this.form).then((res) => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            } else {
  ...
  created() {
    this.getEmployeeDocuments(this.form.parameterTypeId);
  }
}
</script>