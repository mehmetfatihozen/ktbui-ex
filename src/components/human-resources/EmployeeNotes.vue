<template>
  <div>
    ...
    >
      <div class="form-group">
        ...
        <template v-if="$v.form.title.$error">
          <small class="text-danger" v-if="!$v.form.title.required">{{ this.getMessage(165) }}</small>
          <small class="text-danger" v-else-if="!$v.form.title.checkTitle">{{ this.getMessage(166) }}</small>
        </template>
      </div>
      <div class="form-group">
        ...
        <template v-if="$v.form.description.$error">
          ...
          <small class="text-danger" v-else-if="!$v.form.title.checkDesc">{{ this.getMessage(167) }}</small>
        </template>
      </div>
      <div class="row">
        ...
        <div class="col-3">
          <div class="form-group ">
            ...
            <template v-if="$v.form.dateTime.$error">
              <small class="text-danger" v-if="!$v.form.dateTime.requiredIf">{{ this.getMessage(168) }}</small>
              <small class="text-danger" v-if="$v.form.dateTime.requiredIf&&!$v.form.dateTime.checkDate">{{ this.getMessage(228) }}</small>
            </template>
          </div>
        </div>
      </div>
      <div class="form-group">
        ...
        <template v-if="$v.form.receiverTo.$error">
          <small class="text-danger" v-if="!$v.form.receiverTo.requiredIf">{{ this.getMessage(90) }}</small>
        </template>
      </div>
      <div class="form-group">
    ...
    <table class="table table-hover">
      ...
      <tbody>
      <tr class="cursor-pointer" @click="showEditModal(item.id)" v-for="(item,index) in $store.state.hr.notes"
          :key="index">
        <td>{{ item.konu }}</td>
...
export default {
  ...
  methods: {
    ...
    ...mapActions('hr', ['updateNote', 'getNoteDetail', 'deleteNote', 'addNote', 'getNotes']),
    clear() {
    ...
    save() {
      ...
      if (!this.$v.$invalid) {
        ...
        if (this.isEditing) {
          this.updateNote(this.form).then((res) => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            } else {
          ...
          this.addNote(this.form).then((res) => {
            ...
            if (res.hasError) {
              ...
              this.$bvModal.hide('modal-add-edit');
              this.showModal({title: 'Başarılı!', message: this.getMessage(161), type: 'success'});
            }
          }).catch(err => {
            this.overlay = false;
            this.showModal({title: 'Hata!', message: err, type: 'danger'});
