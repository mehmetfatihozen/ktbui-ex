<template>
  <div>
    <b-modal
        id="modal-annualBudgetYears-add-edit"
        centered
        :title='modalTitle'
        ok-variant="success"
        ok-title="Kaydet"
    ...
    <table class="table table-hover">
      ...
      <tr>
        ...
        <th class="fit text-right"> <PlusCircleIcon v-b-modal.modal-annualBudgetYears-add-edit/> </th>
      </tr>
      </thead>
      <tbody>
      <tr class="cursor-pointer" @click="showEditModal(item)" v-for="(item,index) in $store.state.budgetManagement.annualBudgetYears"
          :key="index">
        <td>{{ item.yil }}</td>
        <td class="">
...
export default {
  ...
  methods:{
    ...mapActions('budgetManagement',['changeAnnualBudgetStateTransitionsUserAssignment', 'getAnnualBudgetYears','addAnnualBudgetYears','updateAnnualBudgetYears']),
    ...
    save(){
      ...
      if (!this.$v.$invalid) {
        this.overlay = true;
        if(this.isEditing){
          this.updateAnnualBudgetYears(this.form).then((res) => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            } else {
              this.$bvModal.hide('modal-annualBudgetYears-add-edit');
          ...
          this.addAnnualBudgetYears(this.form).then((res) => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            } else {
    ...
    showEditModal(item) {
      ...
      this.$bvModal.show('modal-annualBudgetYears-add-edit');
    },
    clearForm(){
      this.modalTitle = 'Yıl Girişi';
      this.$v.$reset();
