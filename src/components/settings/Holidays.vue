  ...
  <div>
        ...
        <div class="col">
          <div class="form-group">
            ...
            <template v-if="$v.form.title.$error">
              <small class="text-danger" v-if="!$v.form.title.required">Resmi Tatil Adı girilmelidir</small>
            </template>
          </div>
        </div>
    ...
    <table class="table table-hover">
      ...
      <tr>
        ...
        <th class="fit"> <PlusCircleIcon v-b-modal.modal-holidays-add-edit/> </th>
      </tr>
      </thead>
      <tbody>
      <tr class="cursor-pointer" @click="showEditModal(item)" v-for="(item,index) in $store.state.settings.holidays" :key="index">
        <td>{{item.metin}}</td>
...
export default {
  ...
  methods:{
    ...mapActions('settings',['getHolidays','addHoliday','updateHoliday']),
    ...
    save(){
      ...
      if (!this.$v.$invalid) {
        this.overlay = true;
        if(this.isEditing){
          this.updateHoliday(this.form).then((res) => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            } else {
              this.$bvModal.hide('modal-holidays-add-edit');
          ...
          this.addHoliday(this.form).then((res) => {
            this.overlay = false;
            if (res.hasError) {
              this.showModal({title: 'Hata!', message: res.errorMessage, type: 'danger'});
            } else {
    ...
    showEditModal(item) {
      this.modalTitle = 'Resmi Tatil Düzenle'
      this.isEditing = true;
      this.form.id = item.id;
      this.form.title = item.metin;
      this.form.passive = item.pasif;
      this.form.parametreTipId = 17;
      this.$bvModal.show('modal-holidays-add-edit');
    },
    clearForm(){
