<template>
  <div class="d-flex flex-fill justify-content-center align-items-center">
    <form class="form-sms d-flex justify-content-center align-items-center flex-column" @submit.prevent="login">
      <p class="text-center font-weight-bold">Doğrulama Kodunu Giriniz</p>
...
export default {
  ...
  methods: {
    ...
    ...mapActions('auth', ['updateToken', 'checkSms', 'resetLoginState']),
    ...
    login() {
      if (this.number.length > 5) {
        this.checkSms({userId: this.$store.state.auth.user.id, code: this.number}).then((res) => {
          if (!res.hasError) {
        ...
        this.showModal({title: 'Hata!', message: 'SMS şifresini giriniz', type: 'danger'});
      }
    },
    updateRules(items) {
